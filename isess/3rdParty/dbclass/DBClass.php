...
/**
 * DBClass for connecting to any DB server that is PDO compartible
 * The class now supports use as a singleton and as an instance class
 * */
class DBClass {
    ...
    /**
     *
     * @var string The port number for accessing the server 
     */
    protected $port;
    ...
    /**
     * @var string PDO compartible name of the DB server  
     * */
    protected $dbserver;
    ...
    /**
     * @var PDO The instance of the connection that will be used by this class 
     * */
    public $conn;

    /**
     * @var PDOStatement This will refer to an instance of PDOStatement created by the class
     * */
    private $stmnt;
    ...
    /**
     * Class Constructor
     * Initializes an instance of this class to the default params. 
     * It will also attempt to connect to the DB server if autoconnect is set to true
     ...
     * @param string $dbserver The name of the DB server in PDO compartible form e.g. mysql
     * @param string $host
     * @param string $user
     * @param string $pass
     * @param string $dbname
     ...
     * @return \DBClass
     */
    public function setConnectionParams($dbserver, $host, $user, $pass, $dbname, $port = "", $prefix = "") {
        ...
        $name = (!empty($prefix) ? $prefix . '_' : '') . $dbname;
        $this->dbname = $name;
    ...
    /**
     * Connects to the database server. Throws an exception if the connection attempt fails
     * @throws  \ConnException
     */
...
//    }
    ...
    public function execNonQuery($sql) {
        ...
        if (!preg_match("/^(" . implode('|', $allowables) . ")\\s+/i", $sql)) {
            ...
            try {
                ...
                throw new QueryException($errInfo[2], $errInfo[0], $sql, 'An error occured while communicating with the DB');
            }
        }
    }
     ...
     * @return array The column details as an array
     */
    public function getColumnsTypeInfo($query) {
        // Get the column count
        $colCount = $this->countCols($query);

        try {
            $stmnt = $this->conn->query($query);
            /** @var array Aggregated column details */
            $columnDetails = array();
            // Iterate over the columns and get the column types
            for ($i = 0; $i < $colCount; $i++) {
                //TODO: replace this with a method that generates the meta information compartible with all drivers
                $meta = $stmnt->getColumnMeta($i);

                //TODO: Determine if PDO type should be converted
                $columnDetails[$i] = array(
            ...
            throw new QueryException($pex->getMessage(), $pex->getCode(), $query, 'Column details could not be retrieved');
        }
    }
